...
class Users
{
        ...
        switch($reg_type){            
            ...
            case 'account': {
                ...
                if($account_type !== 'account'){
                    return ['ret'=>-1, 'msg' => '账号格式不正确'];
                }
                if($account_type == 'mobile'){
                    return ['ret'=>-1, 'msg' => '账号是手机格式，请用手机账号模式注册'];
                }                       
                $array = $this->db->select('uid')->from($tablename)->where('account = ?', $account)->get();
                if(count($array) > 0){
                    return ['ret'=>-1, 'msg' => '账号已存在'];
                }                 
                break;
            }
        }
    ...
    public function login($account, $pwd){
        ...
        $rows = $this->db->select('uid', 'role')
            ->from('t_admin')
            ->where("uid = '{$ret['uid']}' and isdel = 0")
            ->get();

        if(count($rows) == 1){
            $ret['role'] = $rows[0]['role'];
        }
    ...
    public function update($key, $val, $clientuid){
        ...
        // 自己定义头像修改成系统头像时， 删除OSS上的对象
        if($key == 'avatar' && intval($val) < 100) {
            $rows = $this->db->select('avatar')->from('t_users')->where(['uid'=> $clientuid])->get();
            $oldAvatar = intval($rows[0]['avatar']);
            if ($oldAvatar > 100) {
                $this->delOSSAvatarObject($oldAvatar);
            }
        }
    ...
    // 自己定义头像修改成系统头像时， 删除OSS上的对象
    private function delOSSAvatarObject($avatar) {

        $groupId = floor(floatval($avatar) / 1000) * 1000;
        $object = "avatar/{$groupId}/{$avatar}.png";
