...
class Articles
{
     ...
     * @route GET /{Id}
     * @hook \App\Hooks\BasicAuth
     * @return string json:{Id,title,content,type,authorId,status,writetime}
     */
     ...
     * @return string json:{success:1, status:2} | {error:''}
     */
     ...
     * @hook \App\Hooks\BasicAuth
     * @return string json:{success:1}
     */
    public function remove($Id){        
        $ret1 = $this->db->select(['writetime'])->from('t_articles')->where("Id = {$Id}")->get();
        $ret2 = $this->db->select(['fname'])->from('t_articles_oss')->where("AId = {$Id}")->get();        
        ...
        return ['success' => 1, 'writetime' => $ret1[0]['writetime'], 'files' => $ret2];
    }
} 